...
import DeleteExercisesInput from './inputs/delete-exercises.input';
...
export class ExercisesResolver {
  ...
  async createExercise(@Args('exercise') exerciseData: CreateExerciseInput): Promise<Exercise> {
    return await this.service.createExercise(exerciseData);
  }
  ...
  @Roles(USER_TYPE.PERSONAL)
  @UseGuards(GqlAuthGuard, GqlRolesGuard)
  deleteExercise(@Args() args: DeleteExercisesInput){
    const { ids } = args
    try {
      this.service.deleteExercise(ids);
      return true;
    } catch (error) {
      return false;
    }
